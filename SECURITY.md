...
目前為 0.1.3 個人維護的公開資料查詢工具。套件內附合成資料，經審核的正式 snapshot 另由 GitHub Releases 散布。維護者優先處理預設分支的問題；未承諾醫院正式部署、臨床適用或舊版本支援。
...
套件預設不記錄。設了 `TAIWAN_LAB_HTTP_USAGE_LOG` 才會開。
...
任何公開端點都不要輸入病人資料、院內資料、帳號密碼或任何機密內容。要自己掌握紀錄就裝在自己電腦上，本機 stdio 不經任何網路服務。
