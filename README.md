...
1. **競品調查**：食藥署查詢網站一個字打錯、或不知道正式品名就找不到。接上 MCP 後用模糊說法問，像「HPV 的檢測試劑有哪幾家有證」，AI 自己比對品名與效能，幫你鎖定最相關的幾張。
2. **自家證的效期盤點**：列出公司名下全部許可證，依效期排序，抓出半年內要展延的。
3. **新供應商查核**：先查對方名下有幾張證、有沒有註銷紀錄，再決定要不要往下談。
4. **業務拿來的許可證要確認**：證號查真偽、看在誰名下、有沒有註銷或快到期。
5. **院內做不了的傳染病項目，要送哪一家**：直接問哪些認可檢驗機構能做，名單跟範圍一次列出。
6. **特殊檢體不知道怎麼送**：以前翻疾管署手冊翻半天。現在問一句，檢體、容器、運送條件直接給你，附頁碼。
7. **新品送件前研究**：看同類產品的分類代碼與效能描述怎麼寫，找 predicate device。
8. **申報條件搞不清楚**：這個項目多久能報一次、有什麼限制，直接問支付規範。
9. **健保申報要報哪個代碼、幾點**：某個項目或方法對應的代碼跟點數，不用再翻健保署那包檔案。
