...
這個端點在 2026-09-22 從 VM 外部以真正 MCP client 驗證過 24 個工具與四組可追溯的正式 snapshot。主機端有 supervisor 看著服務，程式異常結束會自動重啟，主機重開也會跑一次自癒腳本。

資料每天台北時間 10:30 自動對一次最新的 Release，有新版才換，換完自己重啟服務並打一次 `initialize` 自我檢查。每個下載檔都比對 SHA-256，對不上就整個中止、保留原本的資料，服務不會中斷。狀態寫在主機上的 `update-status.txt`，第一行就是 OK 或異常，`grok-production-status.sh` 也會印出來。

這個更新吃的是本專案的 GitHub Release，而 Release 由專案負責人電腦上每天 09:30 的官方版本檢查產生。那台沒開機就不會有新的 Release，主機這邊也就沒有東西可換。

匿名流量有每來源 IP 限流、256 KiB 請求上限及不快取回應；private deployment 另可要求 bearer token。

**這個公開端點會記錄使用情形。** 每一次呼叫都會寫下時間、來源 IP、用了哪個工具、送進來的查詢內容，以及回應狀態；被限流或擋下來的也會記。維護者用這些資料看有多少人在用、大家在查什麼、哪些問法查不到。記錄不含請求標頭，所以 bearer token 不會被寫進去。這是這台主機的設定，**套件本身預設不記錄**，要設 `TAIWAN_LAB_HTTP_USAGE_LOG` 指定檔案路徑才會開。

換句話說：**架這台主機的人看得到你查了什麼。** 公開端不應接收病人資料、院內資料、帳號密碼或其他機密內容。結果僅供公開資料查找，不能代替採檢、申報、採購或醫療決策。資料庫、排程與 MCP 查詢服務是 owner 自有工作流程；Grok Bot 條款只明確寫 internal business purposes，因此目前保留的條款不確定性只針對匿名公開端點是否落在該用語內，不把它誤寫成「代管資料庫」或已知違規，詳見[接手事項](docs/next-steps.md)。
