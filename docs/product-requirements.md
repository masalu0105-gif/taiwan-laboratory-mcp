...
本表是 P1.1 public MCP contract 的封閉 operation registry；公開集合必須與表中 24 個 exact names 完全相等（~~22 個~~ 2026-09-16 owner 為手冊第 7 章選一個工具，加入 `get_testing_location`，OD-18；同日再加手冊第 3–6 章的 `search_manual_procedure`，OD-20）。Machine-readable 真源目前已建立於 package resource `src/taiwan_lab_mcp/contracts/public-contract-v1.json`，內容同時包含 operation、request／response schemas、source payload／locator schemas、public enum、freshness、TFDA warning 與 safety registries；source／installed-wheel equality verification 已完成。SDD 可使用不同內部物件，但必須一對一 mapping，TDD 以 package resource 對 MCP `list_tools` 做集合、signature 與 response-schema equality，禁止 repo-relative fallback。
...
| `search_manual_procedure` | active | `query: string` | `cdc_manual` | sample + official | 回手冊裡不是表格列的部分：第 1 章通則、第 3–6 章（採檢步驟、包裝運送、外溢清消、運送箱清消）、第 8 章地理位置、第 9 章屍體解剖、附件一～七表單，以及各章的圖說；文字查詞，也可直接給條號（`3.5` 一併回其下層步驟）；文字照手冊原樣含編號，不與第 2、7 章的表格列合併 |
...
| `not_pre_submission_storage` | `search_disease`、`get_specimen_requirement`、`get_collection_method`、`get_container`、`get_transport_requirement`、`get_submission_rule`、`get_testing_location`、`search_manual_procedure` | 官方「應保存種類（應保存時間）」不是檢體送驗前 storage instruction |
...
| OD-20 | 採檢手冊第 3 到 6 章也要做，之後再擴大為整本做完（owner 2026-09-16「第 3 到 6 章也做」、「全部把他做完 不要再一直問了」） | 手冊裡不是表格列的部分全部收錄：第 1 章、第 3–6 章、第 8、9 章、附件一～七與各章圖說，一條條文／一個圖說／一份附件各存一列，文字保留印出來的編號，存成 `cdc_manual_clause`；第 2、7 章的表格頁只補讀圖說（表格列已另存）；沒有 Word 表格標記可比對，改以「存進去的字和那些頁印出來的字逐字相同」當建置前關卡；查詢新增一個工具 `search_manual_procedure`（operation registry 23→24）；審核規則第 4 版納入第 3–6 章、第 5 版納入其餘章節與附件 |
| OD-21 | 除了裝在自己電腦上，另外做一個「連網址」的接法，讓別人不用安裝就能用（owner 2026-09-17「對，開始寫那一層」） | MCP 官方規格的兩種接法之一（Streamable HTTP）；同一個 `server.mcp`、同一組 24 個 operation、同一份唯讀 snapshot，只換最外層傳輸。新增 console script `taiwan-lab-mcp-http` 與 `src/taiwan_lab_mcp/http_server.py`，設定走 `TAIWAN_LAB_HTTP_HOST`／`_PORT`／`_PATH`／`_ALLOWED_HOSTS`。預設只聽 `127.0.0.1`；要服務其他電腦必須同時給 Host 允許清單（開啟 DNS rebinding 保護）與 `TAIWAN_LAB_DATA_MODE=official_snapshot`，兩者缺一就拒絕啟動（exit 2）。`stateless_http=True`：每個查詢都是唯讀，不需要記住上一次，重開也不會斷線。主機放哪、要不要留查詢紀錄、隱私聲明怎麼寫，尚未決定 | `D-026` |
| OD-22 | 名冊的機構簡稱、許可證的廠商中文名與新冠用詞都要查得到（owner 2026-09-17「三個都做」，起因是 40 題實測有 3 題問法對不上）| 沿用 OD-16 的做法：版本化對照表放在程式包內，只換查詢字，答案仍用資料原本的寫法，不改任何已存的列。`rules/cdc_lab_alias/v1.json` 14 條（成大→成功大學、三總→三軍總醫院…），名冊逐字替換所以「成大 傷寒」會變成「成功大學 傷寒」；`rules/tfda_term_alias/v1.json` 分兩區，廠商 6 條（亞培→Abbott…）只在查製造商欄時套用，產品 4 條（新冠／武漢肺炎／嚴重特殊傳染性肺炎→新型冠狀病毒）只在查品名時套用。收錄標準是實測：替換前後的列數一定要變多，否則不收（「COVID」與「新型冠狀病毒」因此被刷掉）。這兩張表不進 curated build 的 fingerprint，因為它們不改變任何一列的內容。已知限制：一個別名只能換一個字，新冠相關的許可證分散在三種寫法，單一替換覆蓋 25/30 列 | `D-027` |
| OD-23 | TFDA 新冠口語詞改為多關鍵字聯集（owner 2026-09-22「多關鍵字聯集應該可以擴大關鍵字吧？」並同意嘗試） | `tfda-term-alias-v2` 將四個口語別名展開成「新型冠狀病毒」「SARS-CoV-2」「COVID-19」。同一來源列即使命中多個詞也只能回一次；排序取所有變體中的最佳既有 match tier，再以既有取消狀態、許可證字號與來源列順序決定。回傳 query 保留使用者原文，`matched_by` 只列實際命中的來源欄位，不改存放資料、不宣稱臨床等效。 | `D-028` |
