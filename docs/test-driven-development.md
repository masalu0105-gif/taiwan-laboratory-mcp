...
- 實作測試（2026-09-15）：
  ...
  - `tests/test_nhi_autoupdate.py`：同檔不建置；點數變動自動發布（reviewer 為 `automated-check:nhi-auto-update`、protocol `nhi-r1-auto-review`、審核證據含全表逐列比對、驗收題 ≥10、查得到新點數）；沒有檢驗範圍規則的新代碼列在 `new_codes_without_scope`；列數變動超過 10% 擋下且第二天 `already_reported=true`、不多建 build；逐列比對失敗不換版；開發安裝不發布；手動審核 protocol 不變；獨立比對與建置結果一致；CLI `check nhi_fee --auto-publish`。
- 未審核代碼先算「是」（OD-12／D-021）：`tests/test_nhi_autoupdate.py` 驗健保正式建置的新代碼為 `in_scope`、判定依據寫明 owner 決定、查詢 `coverage_status=complete`；`tests/test_tfda_autoupdate.py` 驗附表 A/B/C 未審代碼為 `included` 並列在摘要；`tests/test_tfda_ivd_registry.py` 驗 D 類仍為 `unknown`、未審 A/B/C 碼遇到 excluded 碼為 `ambiguous`。離線合成建置維持 `review_pending`（`tests/test_nhi_scope.py`）。
- 食藥署下載包（OD-10／D-019）：`tests/test_tfda_bundle.py` 驗匯出只含原始 ZIP、`fetch.json` 與 curated build（含 `audit/reviews/`），清單 hash 相符，同一個服務版本匯出兩次 bytes 相同；安裝到空資料夾後 `get_license` 查到同一版、`freshness_policy_version=tfda-v1`、重裝回 `already_installed`；指令來源和檔案不符回 `BUNDLE_SOURCE_MISMATCH` 且不寫任何檔案；上限為壓縮後 128 MiB、解壓後 512 MiB；CLI `export-snapshot`／`install-snapshot tfda_devices`。健保既有的 `tests/test_snapshot_bundle.py` 全部沿用，驗串流改寫沒有改變行為。
  - `tests/test_tfda_autoupdate.py`：同檔不建置；小幅變動自動發布（reviewer 為 `automated-check:tfda-auto-update`、protocol `tfda-r1-auto-review`、審核證據含全表逐列比對、驗收題 ≥10、查得到新值）；列數變動超過 10% 擋下且第二天 `already_reported=true`、不多建 build；新的註銷狀態值擋下；逐列比對失敗不換版；開發安裝不發布；獨立比對與建置結果一致；CLI `--auto-publish` 只限 `tfda_devices`；只有展延、筆數不變的新版也自動發布並查得到新有效日期；清理清單保留最近 3 版與等待中的新版、列出更舊的建置與原始檔和殘留建置、只列清單不刪檔、版本不超過 3 個時不列、CLI `retention-plan`。
  - `tests/test_tfda_publish.py`：AI 代審的正式 build 成功並寫入 protocol `tfda-r1-ai-review`、開發安裝拒絕、reviewer 不是 protocol 指定身分拒絕、驗收題少於 10 題／比對失敗／reviewer 不符拒絕且不寫任何 curated 檔；每日檢查同檔成功、新檔保留舊版並寫差異、下載失敗標 stale、沒有服務中資料只寫報告、CLI `check tfda_devices`；筆數不變時差異報告仍列出內容有改的字號、改動欄位前後值與有效日期往後延（展延）。
...
- 版面讀法依 ADR 0003：欄位只照表頭文字對應（沒有表頭的頁面只在各欄寬度與上一頁相同時沿用上一頁欄位，否則整批擋下），第 2.6 節 7 欄表格沒有「應保存種類（應保存時間）」時該欄回 `null`；紅字修訂底線（紅色、端點沒有貼齊格線）不能當列界線；同一行相鄰兩欄的字要依字元位置分回各欄；頁首溢出文字依 TR 內 TD 順序接回上一頁同一格，TD 數不符、頁中溢出或找不到可接格子時整批擋下。
...
- 已實作（2026-09-15，第 2 章拆表規則，ADR 0003）：`tests/test_cdc_manual_layout.py` 以合成 `CdcLayoutV1`（黑色格線、依 PDF 內順序排列的字元框、Word TR／TD 標記，尺寸取自 1150826 手冊第 14 頁）驗：合併格（疾病、目的、送驗方式、注意事項）的原文帶進每一個採檢項目列；每列定位含 PDF 頁、印刷頁（頁首「頁碼：第 N 頁」）、節名（表格上方「2.N.…」，後續頁沿用）與 row_bbox，row hash 為 8 欄原值；字元依座標分回各格，整頁按行排序仍正確；紅色線與端點沒貼齊格線的黑線不算列界線；7 欄表格保存欄回 `null`；下一頁頂端溢出的「裝)」接回上一頁同一格，兩頁的列都拿到完整原文，空白的疾病、目的、時間、保存欄沿用上一頁；沒有表頭但欄寬相同的頁面當延續頁。擋下：表頭文字不符（`LAYOUT_HEADER_MISMATCH`）、沒有表頭且欄寬不同（`LAYOUT_HEADER_MISSING`）、跨頁欄位數對不起來（`LAYOUT_CONTINUATION_AMBIGUOUS`）、第一張表上方沒有節名（`LAYOUT_SECTION_MISSING`）、找不到表（`LAYOUT_NO_TABLE`）、TR 格數不等於欄數（`LAYOUT_TAG_WIDTH_MISMATCH`）、頁中出現溢出（`LAYOUT_MID_PAGE_OVERFLOW`）、表格頁沒有文字（`LAYOUT_NO_TEXT_LAYER`）。另以故意改壞規則確認測試會失敗：放寬端點容差 1 failed、跨頁不接格 2 failed、表頭不檢查 12 failed。從 PDF 讀出 `CdcLayoutV1` 的 PDFium 部分與真實手冊比對是下一個切片。
- 已實作（2026-09-15，真實手冊試跑後補的拆表規則）：以 PDFium 試用版把 1150826 手冊讀成 `CdcLayoutV1` 交給拆表程式，依序卡在三處，各先補會失敗的測試再修：頁首「編號／版次／頁碼」格的橫線貼齊表格欄線，被當成列線（改為只取表格欄線直線上下範圍內的橫線，範圍外的字才找節名）；「3 mL 血清」裡的空白字元結尾超過欄線，被算到隔壁欄（空白一律跟著前一個字）；一格的標記另列了沒有字、位置框在隔壁欄的 marked content（有可見字時只用字判斷欄位）；另外節名讀成「2.2 第.二類…」（標題行內改照 PDF 內順序，不照 x 排序）。修正後整章讀出 370 列，與只看 Word TR 標記數出的 370 列相同。
- 已實作（2026-09-15，PDFium 讀取進正式程式，`SDD-QUAL-01`）：`tests/test_cdc_pdf_importer.py::test_sdd_qual_01_pdfium_identity_and_resource_resolution` 驗 package resource `qualifier_specs/pdfium-layout-v1.json`（pypdfium2 5.13.0、PDFium 153.0.7999.0、讀表規則 `cdc-manual-layout-v1`）與 extractor identity 相同，版本不符回 `QUALIFIER_IDENTITY_MISMATCH`、沒有安裝回 `QUALIFIER_DEPENDENCY_MISSING`；另以測試內用 pypdfium2 產生的 PDF（黑色與紅色填色線、直線、Helvetica 文字）驗讀出的座標從左上角起算、線的顏色、字元依序與框；不是 PDF 的位元組回 `PDF_UNREADABLE`。Word 表格標記無法在測試內產生，由真實手冊比對涵蓋：正式程式讀出的 370 列 8 欄文字與試用版逐列相同。`pypdfium2` 為 optional extra `cdc-manual`，CI 安裝 `.[dev,cdc-manual]`。
- 已實作（2026-09-15，格內換行的顯示文字，OD-15）：`tests/test_cdc_manual_layout.py` 以合成頁驗 `CdcSpecimenRow.display_fields()`，`fields()` 原文不變：擠到格線才折的行接起來（「病原體檢測；血清」接「型別鑑定」）；英數字兩邊補一個空格（「…收集 3」接「mL 血清」）；還有空間就換的行保留（「傷寒」「副傷寒」）；「1.」「2.」前一律換行，「3.4節」不算編號；「日」單獨放得下但「)」不能放行首，「日)」一起量寬後接回「發病後(30日)內」；單獨一個「。」自成一行、行首是「。」時接回上一行；跨頁溢出的「染性物質)」接回上一頁最後一行，兩列顯示文字相同。故意改壞規則確認會失敗：不處理行首符號、單字行不換行、容差改回 1.5 pt 各至少 1 failed。真實手冊比對寫在 implementation notes。
- 已實作（2026-09-16，頁首版次與核准日期，SDD §10.3）：`tests/test_cdc_manual_layout.py` 的合成頁頁首印「版次：1150826 核准日期：115年08月26日」，拆表結果的 summary 回 `manual_version=1150826`、`approved_date_raw=115年08月26日`（原文，不轉西元）；表格頁缺版次或核准日期回 `LAYOUT_VERSION_MISSING`，任一頁版次或日期和第一頁不同回 `LAYOUT_VERSION_CONFLICT`。先寫測試跑出 4 failed，實作後全過。
- 已實作（2026-09-16，手冊資料庫與 6 個手冊查詢工具接正式資料，SDD §10.3、OD-15）：`tests/test_cdc_manual_official.py` 以合成版面建置（`build_cdc_manual_snapshot`，審核者為 `offline-test-builder`）驗：official 模式 `search_disease("傷寒")` 先回疾病名稱完全相同的列（第 16、17 頁），再回開頭相同的列（第 14 頁「傷寒↵副傷寒」）；record 為顯示文字，evidence 為 `cdc-manual-pdf`、`cdc_pdf_row` 定位與原文算出的 row hash，`not_pre_submission_storage=true`；provenance 為 `cdc_manual`、版次 1150826、`coverage_status=complete`，並附「非疾管署官方服務」註記；`get_specimen_requirement("登革熱")` 回 1 列；「副 傷寒」忽略空白仍查得到，「狂犬病」回 `not_found`；資料庫同時存原文、顯示文字、版次、核准日期、row hash 與 row_bbox；建置指紋的 qualifier 綁 PDFium 規格與版面 hash（座標差 0.0004 pt 不改 hash，差 0.001 pt 會改）；沒有正式資料回 `data_unavailable`／`no_serving_snapshot`，不退回 sample；`get_data_status` 回報版次與 `cdc-manual-v1`，兩天沒檢查標 `upstream_check_overdue`；資料庫被竄改回 `serving_integrity_failure`。先寫測試跑出 6 failed（沒有正式資料的那一個原本就成立），實作後全過。
- 已實作（2026-09-16，手冊正式建置與 Word 標記比對，OD-04、ADR 0003 決定 5）：`tests/test_cdc_manual_official.py` 以 `_write_raw_revision` 寫入兩份合成 PDF 與 fetch.json，PDF 讀取換成回傳合成版面（測試內確認讀的是手冊、不是修訂對照表），驗 `build_official_cdc_manual_snapshot`：四關審核紀錄的 reviewer 是 `ai-reviewer:claude-opus-5`、protocol 是 `cdc-manual-r1-ai-review`／1，證據清單含手冊 PDF、修訂對照表 PDF、fetch.json 與 Word 標記比對報告（4 列、32 格、沒有不符）；certificate `approved`、10 題；發布後查得到。擋下且不寫 current：驗收題只有 9 題、預期值不符、驗收題的疾病名稱不在那一列、reviewer 不是 protocol 指定的人、有 major finding、附件版本 1150827 但頁首印 1150826（`MANUAL_VERSION_MISMATCH`）、原始 PDF 和 fetch.json 對不上（`RAW_REVISION_INTEGRITY`）、不是安裝版（`APPLICATION_BUILD_IDENTITY_MISSING`）；把「尿液」的「液」往下移到下一列時，格線拆出「尿」與「液血清」，Word 標記仍是「尿液」「血清」，回 `LAYOUT_TAG_TEXT_MISMATCH`。先寫測試跑出 9 errors、再補比對測試跑出 2 failed，實作後全過。
- 已實作（2026-09-16，手冊每日自動更新，OD-04）：`tests/test_cdc_manual_autoupdate.py` 以合成官網頁、viewer 與兩份合成 PDF（PDF 讀取換成回傳合成版面）驗 `run_cdc_manual_auto_update`：和服務中同一個 raw revision 回 `unchanged`、generation 2、不過期；手冊重新上傳且登革熱採檢量「3 mL」改成「5 mL」時自動換版（`published`），差異摘要寫「內容有改的列：1」並列出登革熱，查詢回新值，審核紀錄 reviewer 為 `automated-check:cdc-manual-auto-update`、protocol `cdc-manual-r1-auto-review`，證據含 Word 標記比對；新版只剩 1 列時擋下（`row_count_change_exceeds_10_percent`、`disease_count_change_exceeds_10_percent`），隔天同一新版回 `already_reported=true`，服務中版本不變並標 `newer_candidate_pending_review`；讀手冊的規則版本不同時擋下（`manual_reading_rules_changed`）；新版格線與 Word 標記不符時 `auto_publish_failed`／`LAYOUT_TAG_TEXT_MISMATCH`，服務中版本不變；官網打不開回 `failed`／`discover` 並標 `upstream_verification_failed`，查詢照舊回答；沒有服務中手冊回 `no_serving_snapshot` 且不建立 current；CLI `check cdc_specimen_manual --auto-publish` 呼叫自動更新、帶 `--publisher-oid` 時拒絕。先寫測試跑出 8 failed（沒有自動更新模組），實作後全過。
- 已實作（2026-09-16，常見病名對照表，OD-16）：`tests/test_cdc_manual_official.py` 驗查「typhoid」回和查「傷寒」一樣的三列（第 16、17、14 頁，完全相同的疾病名稱排在包含它的前面）、查「dengue」回 1 列、查「malaria」（合成手冊沒有這個疾病）回 `not_found`；對照表版本與檔案 hash 進 build fingerprint 的 `rules`（`cdc_disease_alias`／`cdc-disease-alias-v1`）。先寫測試跑出 2 failed，實作後全過。對照表本身的 95 個別名逐一對真實手冊查過，全部都查得到列（寫在 implementation notes）。
- 已實作（2026-09-16，疾管署下載包，OD-19、ADR 0004）：`tests/test_cdc_bundle.py` 驗名冊與手冊各自匯出的下載包：名冊包含 `source.ods` 與 `fetch.json`，手冊包含 `manual.pdf`、`revision.pdf` 與 `fetch.json`，兩者都含 curated 資料庫；bundle manifest 帶顯名與「非疾管署官方服務」說明。安裝到另一個 data root 後，名冊查「傷寒」回 2 筆、手冊查「登革熱」回 1 筆（檢體「血清」），狀態 available、`freshness_policy_version` 分別是 `cdc-labs-v1`、`cdc-manual-v1`。CLI `export-snapshot` 兩個來源都 exit 0，`install-snapshot` 接受這兩個來源（檔案不存在時錯在檔案，不是來源名稱）。審核紀錄的 protocol 版本改為第 2 版（下載包納入範圍）。先寫測試跑出 3 failed，實作後全過。
- 已實作（2026-09-16，手冊第 7 章拆表，OD-18、ADR 0003）：`tests/test_cdc_manual_chapter7.py` 用合成版面驗第 7 章三種表各自的欄位：送驗地點表 8 欄（傳染病名稱、採檢單位、採檢項目、檢驗方法、檢驗期限、收件單位、實驗室生物安全等級(BSL)、備註），合併格會照第 2 章的規則往下帶（天花的採檢單位、檢驗期限、BSL 帶到第二列）；7.7 屍體解剖檢體印「檢驗期間」而且沒有 BSL 欄，讀成 `testing_period_raw`、`turnaround_raw` 與 `bsl_raw` 都是 null；7.9 收件單位聯絡方式讀成 `單位名稱／電話／傳真／地址` 四欄；同一份版面裡三個 parser 只讀自己那種表（第 2 章只讀第 14 頁、第 7 章讀第 93、120 頁、7.9 讀第 121 頁）；欄名對不上任何一種表仍回 `LAYOUT_HEADER_MISMATCH`。先寫測試跑出 5 failed（`parse_cdc_testing_locations` 還不存在），實作後全過。真實手冊（1150826）實跑：第 2 章仍是 370 列、55 頁、127 個跨頁接續（和改版前相同），第 7 章 217 列（第 93–120 頁，其中 7.7 一列）、收件單位 4 列（第 121 頁）。
- 已實作（2026-09-16，第 7 章寫進資料庫並納入標記比對，OD-18）：`tests/test_cdc_manual_official.py` 的合成手冊版面加入第 7 章三頁，驗建置後資料庫另有 `cdc_testing_location`（3 列：天花兩種檢驗方法、7.7 屍體解剖；7.7 那列的 `turnaround_raw` 與 `bsl_raw` 是 null、`testing_period_raw` 有值）與 `cdc_receiving_unit`（2 列，含 `unit_search`）兩張表，各自帶原文、顯示文字、頁碼、章節、版次與 row hash；沒有第 7 章的手冊回 `LAYOUT_NO_TABLE` 不建置；Word 標記比對報告改成逐表列出（4 列 32 格、3 列 27 格、2 列 8 格）。先寫測試跑出 3 failed，實作後全過（全部 572 passed）。真實手冊（1150826）實跑標記比對：第 2 章 370 列 2,960 格、第 7 章 218 列 1,962 格、收件單位 4 列 16 格，全部相同。第 7 章的列界線改成每一欄都算：第 101 頁有一組只差在檢驗期限的安排（Sanger 定序 2-7 工作日／目標次世代定序 10-15 工作日），只看五個欄位會少讀一列，Word 標記也算兩列。
- 已實作（2026-09-16，修訂對照表，OD-18）：`tests/test_cdc_manual_revision.py` 用合成橫式版面驗 `parse_cdc_revision_entries`：一筆異動讀成一列（印刷頁碼 6、主旨「傷寒、副傷寒」、說明「修訂採檢項目、採檢目的」），內層巢狀表格的格線不影響外層三欄；沒有說明的區塊算前一筆的延續（記在 `continues_on_pages`），不另開一列；頁碼範圍「(P74-79)」讀成 `74-79`；沒有頁碼的整體性異動（統一用字）頁碼與主旨為 null、說明照存；外層不是三欄回 `LAYOUT_REVISION_SHAPE`；第一個區塊就沒有說明回 `LAYOUT_REVISION_ORPHAN_ROW`。先寫測試跑出 4 failed，實作後全過。真實修訂對照表（1150826，29 頁）實跑：20 筆異動、製表日期 115年08月26日；其中 19 筆有印刷頁碼，拿去比對已讀出的手冊列，16 筆在該頁找得到同名疾病，另外 3 筆是章節層級的異動（3.18 驗餘檢體、4.4 包裝及運送、4.6 不良檢體判定標準），那幾章本來就不在資料庫裡。
- 已實作（2026-09-16，修訂對照表進稽核證據，OD-18）：`tests/test_cdc_manual_official.py` 的正式建置測試把修訂對照表 PDF 也對應到一份合成版面，驗證據清單多一項 `cdc-manual-revision-changes`，內容為檢查代號 `cdc-manual-revision-change-list-v1`、手冊核准日期與修訂表製表日期（都是 115年08月26日）、以及那一筆異動（頁碼 6、傷寒、副傷寒、修訂採檢項目、採檢目的）。先改測試跑出 1 failed，實作後全過（全部 576 passed）。真實修訂對照表實跑：證據檔 3,511 bytes、20 筆異動。
- 已實作（2026-09-16，自動換版也看第 7 章，OD-18）：`tests/test_cdc_manual_autoupdate.py` 驗新版手冊的第 7 章送驗地點少掉三分之二的列時擋下，理由碼帶表名 `cdc_testing_location:row_count_change_exceeds_10_percent`、`cdc_testing_location:disease_count_change_exceeds_10_percent`，服務中版本不變；新版手冊整個沒有第 7 章時回 `auto_publish_failed`／`LAYOUT_NO_TABLE`，同樣不換版；第 2 章自己的理由碼維持原樣不加表名。先寫測試跑出 1 failed，實作後全過（全部 579 passed）。
- 已實作（2026-09-16，第 7 章查詢工具，OD-18，owner 選 B「一個工具就好」）：`tests/test_cdc_manual_official.py` 驗 `get_testing_location("天花")` 回 2 列，第一列的九個欄位與頁面一致，並附 7.9 南港臨時辦公室的電話、傳真、地址；證據列出第 93 頁第 7 章那一列與第 121 頁 7.9 那一列各自的頁碼與章節；7.7 的列 `testing_period_raw` 有值、`turnaround_raw` 與 `bsl_raw` 為 null；查「狂犬病」回 not_found；收件單位名稱在格子裡被折成兩行時仍然對得到該筆聯絡方式。公開契約由 22 個 operation 改為 23 個，`tests/test_p1_1_red.py` 與 `tests/test_mcp_stdio.py` 的工具集合、response schema 與 sample 模式呼叫一併更新，sample 模式用新增的 `data/cdc_testing_location.sample.json`。先寫測試跑出 2 failed，實作後全過（全部 581 passed）。真實手冊實跑：查登革熱回 4 列，每列都附南港臨時辦公室與南區實驗室兩筆聯絡方式；查 COVID-19（別名）也回 4 列。
- 已實作（2026-09-16，第 3–6 章讀成條文，owner「第 3 到 6 章也做」）：`tests/test_cdc_manual_clauses.py` 用合成版面驗：每一條編號段落讀成一列（3、3.1、3.1.1、3.1.1.1），被頁寬折斷的行接起來、中英文交界才補空格，圖說自己一列；跨頁的條文仍是一列並記下續頁；「3.2 抗凝固全血」用空白代替點也算條文；「(圖 3.3B)，炭疽病…」這種折行開頭不算新條文；編號倒退回 `LAYOUT_CLAUSE_ORDER`；第 2 章的圖說在同一頁也不會被當成第 3 章。`tests/test_cdc_manual_official.py` 另驗建置後資料庫多一張 `cdc_manual_clause`，以及逐字比對檢查 `verify_cdc_manual_clause_coverage` 回 0 不符。先寫測試跑出 4 failed，實作後全過（全部 587 passed）。真實手冊實跑：第 3–6 章 143 列（119 條 + 24 個圖說），第 69–92 頁逐字比對 13,943 字元完全一致。
- 已實作（2026-09-16，第 3–6 章查詢工具，OD-20）：`tests/test_cdc_manual_official.py` 驗 `search_manual_procedure("全血")` 回 3.1、3.2 兩條，第一條的記錄含 block_kind、條號、章別與原文（「3.1.全血（whole blood）」），證據帶第 69 頁與章節名；查條號「3.1」回 3.1、3.1.1、3.1.1.1 與圖 3.1；查「狂犬病」回 not_found。公開契約由 23 個 operation 改為 24 個，sample 模式用新增的 `data/cdc_manual_clause.sample.json`。先寫測試跑出 1 failed，實作後全過（全部 588 passed）。真實手冊實跑：查「糞便檢體」「不良檢體」「4.6」「運送箱」都回到對應條文，4.6 的表格欄位以「｜」分開。
- 已實作（2026-09-16，整本手冊做完，OD-20，owner「全部把他做完」）：`tests/test_cdc_manual_clauses.py` 增驗第 1 章、第 8 章與附件一：條號與章別各自正確（1、1.1、8、附件一），附件的 `block_kind` 是 `appendix`、章別是「附件」；第 2 章的圖說（圖 2.5）在同一頁也被讀成自己一列、章別「2」，不再被丟掉。`tests/test_cdc_manual_official.py` 驗建置後 `cdc_manual_clause` 依序含這些列，逐字比對的頁面清單涵蓋第 11、69、70、122、124 頁，並驗 `search_manual_procedure` 查得到「名詞解釋」（1.1）與「附件一」。全部 589 passed。真實手冊實跑：非表格內容 177 列（第 1 章 13、第 3 章 91、第 4 章 49、第 5 章 9、第 6 章 1、第 8 章 1、第 9 章 2、附件 8、第 2 章圖說 3），逐字比對 19,044 字元完全一致。
...
- 已實作（2026-09-15，自動下載）：`tests/test_cdc_ods_source.py` 以仿官方認可機構頁的 synthetic HTML 驗附件依名稱「傳染病認可檢驗機構名冊<7 碼版本>.ods」挑選（旁邊全站共用的 `File/Get` 連結不會被選中）、沒有／兩份名冊／連到其他網站／不是 `File/Get` 連結都擋下；下載成功保存 `raw/cdc_authorized_labs/<raw revision>/artifacts/source.ods` 與 `fetch.json`（含頁面 SHA-256 與版本）並寫 staged 報告，不建立 curated 或 current；只有頁面改動、名冊 bytes 相同時沿用同一 raw revision 且不改寫 `fetch.json`；下載到網頁（content type）或下載檔名與附件名稱不同時在 fetch 階段失敗且不留 raw；工作表名稱與附件版本不一致（`ODS_VERSION_MISMATCH`）或不是 ZIP 時保留 raw、驗證失敗；官方頁 HTTP 失敗在 discover 階段失敗；CLI `sync cdc_authorized_labs` 必須帶 `--upstream`，exit 0／3／4。
- 已實作（2026-09-15，建資料庫與查詢）：`tests/test_cdc_labs_official.py` 以 12 列 synthetic 名冊驗 LAB-01／02 的 MCP 結果（同證號兩個方法各自一筆、12 欄原值、`ods_row` 定位帶真實工作表名稱與列號、row hash、`does_not_confirm_current_acceptance`、非官方與不保證收件說明、出處開頭為提供機關、`get_lab_scope` 相同結果）；比對順序（證號或疾病代碼完全相同在前、只是包含在後）與縣市篩選（臺／台視為相同）；查無回 `not_found`；LAB-04 能力試驗三種原值經 MCP 原樣回傳；沒有正式名冊時回 `data_unavailable` 且不退回 sample、採檢手冊工具仍不可用；`get_data_status` 可用、`coverage_status=complete`、超過 2 天沒有成功檢查標 `upstream_check_overdue`；資料庫被改就不服務；正式建置照 `cdc-labs-r1-ai-review` 第 1 版寫審核紀錄與 10 題驗收證書；驗收題少於 10 題、比對不符、reviewer 不是規則檔指定身分、有 major finding、開發安裝都在寫入前擋下。
- 已實作（2026-09-15，每日自動更新，OD-04 A）：`tests/test_cdc_labs_autoupdate.py` 以 12 列 synthetic 名冊與仿官方頁驗：名冊 bytes 相同只記一次成功檢查（generation +1、不建新版）；新版（同證號一個方法改名）通過全部自動檢查就自動換版，審核紀錄 reviewer 為 `automated-check:cdc-labs-auto-update`、照 `cdc-labs-r1-auto-review` 第 1 版、證據含 `cdc-labs-auto-roundtrip`、驗收證書至少 10 題，差異摘要列出新舊名冊版本，MCP 查得到新方法與新版本；資料列數變動超過 10% 或住址空白比例上升超過 2 個百分點時不換版、標「有新版等待審核」，第二天同一新版標 `already_reported` 且不重建；另寫的逐列比對失敗、開發安裝時都不換版；官方頁打不開標 `upstream_verification_failed`；沒有服務中名冊只回報；另寫的解析器與正式資料庫 12 列比對一致；CLI `check cdc_authorized_labs --auto-publish` 不接受 `--publisher-oid`，健保仍必須帶。
- 已實作（2026-09-15，認可機構查詢翻頁，OD-13）：`tests/test_cdc_labs_official.py` 驗 `find_authorized_lab` ~~預設一頁 5 筆~~（同日改為 20 筆，見下一條）（`limit=5, offset=0`，`query` 回帶四個參數），「C型肝炎＋新北市」8 筆第一頁 5 筆 `truncated=true`、`offset=5` 回 3 筆 `truncated=false`，兩頁合起來依原排序；翻過最後一筆（`offset=8`）仍回 `total_matches=8`；`limit` 為 0、6、字串、布林，`offset` 為負數、字串、null 都回 `invalid_request`；比對順序測試改為逐頁翻完再比；`get_lab_scope` 固定回第一頁 5 筆。MCP discovery 與 public contract 的 request schema 相等（`tests/test_mcp_stdio.py`）。
- 已實作（2026-09-15，一次 20 筆與空格拆詞，OD-13／OD-14）：`tests/test_cdc_labs_official.py` 驗預設 `limit=20`（範圍 1–20），`limit=21` 回 `invalid_request`，`limit=5` 仍可翻頁，`get_lab_scope` 回前 20 筆；`query` 以空格（含全形空格）分成多個詞時每個詞都要命中，縣市名稱或去掉「市／縣」的簡稱改為縣市篩選：「新北市 C型肝炎」「C型肝炎 新北」各 8 筆、「台北 合成」「臺北市　合成」1 筆、「097036 血清」與「高雄 傷寒 血清」只回 B2 那一列、「合成 醫院」3 筆依證號排序；「台北 合成」另給 `city=新北市`、「台南 梅毒」回 `not_found`；單一詞查詢的既有排序測試不變。
