...
- 實作測試（2026-09-15）：
  ...
  - `tests/test_tfda_autoupdate.py`：同檔不建置；小幅變動自動發布（reviewer 為 `automated-check:tfda-auto-update`、protocol `tfda-r1-auto-review`、審核證據含全表逐列比對、驗收題 ≥10、查得到新值）；列數變動超過 10% 擋下且第二天 `already_reported=true`、不多建 build；新的註銷狀態值擋下；逐列比對失敗不換版；開發安裝不發布；獨立比對與建置結果一致；CLI `--auto-publish` 只限 `tfda_devices`。
