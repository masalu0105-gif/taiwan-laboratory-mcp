...
- 已實作（2026-09-15，認可機構查詢翻頁，OD-13）：`tests/test_cdc_labs_official.py` 驗 `find_authorized_lab` 預設一頁 5 筆（`limit=5, offset=0`，`query` 回帶四個參數），「C型肝炎＋新北市」8 筆第一頁 5 筆 `truncated=true`、`offset=5` 回 3 筆 `truncated=false`，兩頁合起來依原排序；翻過最後一筆（`offset=8`）仍回 `total_matches=8`；`limit` 為 0、6、字串、布林，`offset` 為負數、字串、null 都回 `invalid_request`；比對順序測試改為逐頁翻完再比；`get_lab_scope` 固定回第一頁 5 筆。MCP discovery 與 public contract 的 request schema 相等（`tests/test_mcp_stdio.py`）。
