...
| `SDD-API-01` | `tests/test_mcp_stdio.py::test_sdd_api_01_public_contract_resource_matches_discovery` | installed `public-contract-v1.json`與discovery equality（2026-09-16 起 23 個 tool）；`REL-G1` |
...
Qualifier spec 的唯一位置是 Python package resource `importlib.resources.files("taiwan_lab_mcp").joinpath("qualifier_specs", "pdfium-layout-v1.json")`；repo-relative path與`qualifiers/`別名禁止fallback。該spec以`extra=forbid` schema固定 `pypdfium2` exact version、PDFium build與讀表規則版本~~與容差~~（容差寫在拆表程式，由讀表規則版本 `cdc-manual-layout-v1` 標示；2026-09-15 實作時更正，spec 不放程式不會讀的數字）（ADR 0003 §決定 3）。~~npm package `@llamaindex/liteparse`、exact version `2.0.0`、npm dist integrity、bundle files SHA-256、executable basename與approved argv~~（2026-09-15 由 ADR 0003 取代）。
...
| `D-025` 手冊第3–6章 | Accepted（owner 2026-09-16：「第 3 到 6 章也做」，OD-20） | 手冊裡不是表格列的內容（第1章通則、第3–6章、第8章地理位置、第9章屍體解剖、附件一～七、各章圖說）是編號條文、圖說與表單，不是資料表：各存一列到 `cdc_manual_clause`（block_kind、clause_number、chapter、text），文字保留印出來的編號、被頁寬折斷的行接起來、跨頁算同一條。判斷規則：編號後必須接點或空白、單一數字的章號必須印點、目錄的點點引導行不算、表格格子裡的編號算表格內容、讀到第7章就停。章節內的小表格（4.6 不良檢體判定標準）以 Word 表格標記分欄，欄間用「｜」、每列一行。附件表單本身也是 Word 表格，同樣以「｜」分欄保留。第2、7章的表格頁只補讀圖說，表格列不重複存。沒有標記可逐格比對，改以逐字比對當發布前關卡（`verify_cdc_manual_clause_coverage`，1150826 版 19,044 字元相同）。查詢新增 `search_manual_procedure`（文字或條號），不與第2章合併。見 PRD OD-20 |
| `D-026` 用網址連的接法 | Accepted（owner 2026-09-17：「對，開始寫那一層」，OD-21） | `http_server.build_http_app()` 直接掛同一個 `server.mcp`，所以 operation registry 不可能在兩種接法之間分岔（`tests/test_http_transport.py` 以真的 HTTP 來回比對 24 個名稱，並實際呼叫 `get_data_status`）。`stateless_http=True`，因為每個 operation 都是對 snapshot 的唯讀查詢，沒有跨請求狀態。fail-closed：`HttpSettings.check()` 在 host 不是 loopback 時，缺 `TAIWAN_LAB_HTTP_ALLOWED_HOSTS` 或 mode 不是 `official_snapshot` 一律拒絕啟動，因為這兩種錯都不會有錯誤畫面——前者讓任何解析到這台機器的名稱都能驅動它，後者會把合成 fixture 送給別人。有允許清單時開啟 DNS rebinding 保護，`allowed_origins` 取同一批名稱的 https。沒有新增套件依賴（`uvicorn`、`starlette`、`httpx2` 隨 `mcp>=2.2.0` 進來）。主機位置與隱私聲明仍未決定，見 PRD OD-21 |
| `D-027` 查詢用的別名對照表 | Accepted（owner 2026-09-17：「三個都做」，OD-22） | `rules/aliases.py` 讀兩份版本化 JSON，壞檔就丟 `AliasRulesError`，不默默查不到。名冊逐字替換（`apply_lab_aliases`），因為 `search_labs` 本來就以空白拆詞。食藥署的替換做在 `SearchRequest.__post_init__`，不是做在 SQL：adapter 另外用同一個 query 算「命中哪個欄位」，只在 SQL 換會讓查到的列報出空的 `matched_by` 而觸發 model 驗證失敗（實作時實際踩到）。廠商表只在 `fields == ("manufacturer",)` 時套用，因為「亞培」查品名本來就有 1,074 列，替換成 Abbott 反而變差。兩張表都不進 build fingerprint：它們只改查詢字，不改任何已存的列 |
| `D-024` 手冊第7章與修訂對照表 | Accepted（owner 2026-09-16：「手冊第七章也都必須要做」，OD-18） | 手冊裡每一種表寫成一份欄位規格，curated build 一種表一張 SQLite table：`cdc_specimen_requirement`（第2章）、`cdc_testing_location`（第7章送驗地點）、`cdc_receiving_unit`（7.9 收件單位）。~~`cdc_testing_period` 第7.7節獨立schema~~ 更正：7.7 與第7章其他節只差在印「檢驗期間」而非「檢驗期限」、且沒有BSL欄，同一張表多兩個欄位即可，不另開table；兩個官方欄名各自獨立，不互相代用。第7章每一欄都算列界線（1150826第101頁有一組只差在檢驗期限的安排，Word標記也算兩列）。Word標記逐格比對擴大到三張表。`counts` 仍只描述主表（第2章）以維持發布層共用檢查，另加 `table_counts` 描述每張表。~~`cdc_revision_entry` 進 entity table~~ 更正：修訂對照表只讀成異動清單（印刷頁碼、主旨、說明、延續頁），存成每次正式建置的稽核證據 `audit/evidence/revision-changes.json`，不進服務用資料庫，也不複製「修正規定」「現行規定」兩欄的內文；讀者是內容審核（比對自算差異與官方列出的異動）。兩份CDC手冊審核規則出第3版把第7章與修訂對照表納入範圍。查詢工具：owner 2026-09-16 選一個工具 `get_testing_location`（registry 22→23），回第 7 章該疾病的列並附 7.9 收件單位聯絡方式；join 在 query 層做，資料庫不合併，第 7 章列與每筆聯絡方式各自帶 locator 與 row hash。見 `docs/adr/0003-cdc-manual-pdf-layout.md`、PRD OD-18 |
...
| `OD-18` | `D-024` | 手冊第7章已讀入並存進資料庫、修訂對照表已讀成異動清單存為稽核證據（2026-09-16）；owner 選一個工具 `get_testing_location`（registry 22→23） |
| `OD-20` | `D-025` | 手冊非表格內容（第1章、第3–6章、第8、9章、附件一～七與各章圖說）讀成條文存進 `cdc_manual_clause`，以逐字比對當建置關卡，新增 `search_manual_procedure`（registry 23→24），審核規則第4、5版（2026-09-16） |
