...
| `D-025` 手冊第3–6章 | Accepted（owner 2026-09-16：「第 3 到 6 章也做」，OD-20） | 第3–6章是編號條文與圖說，不是表格：各存一列到 `cdc_manual_clause`（block_kind、clause_number、chapter、text），文字保留印出來的編號、被頁寬折斷的行接起來、跨頁算同一條。判斷規則：編號後必須接點或空白、單一數字的章號必須印點、目錄的點點引導行不算、表格格子裡的編號算表格內容、讀到第7章就停。章節內的小表格（4.6 不良檢體判定標準）以 Word 表格標記分欄，欄間用「｜」、每列一行。沒有標記可逐格比對，改以逐字比對當發布前關卡（`verify_cdc_manual_clause_coverage`，1150826 版第69–92頁 13,943 字元相同）。查詢新增 `search_manual_procedure`（文字或條號），不與第2章合併。見 PRD OD-20 |
...
| `OD-20` | `D-025` | 手冊第3–6章讀成條文存進 `cdc_manual_clause`，以逐字比對當建置關卡，新增 `search_manual_procedure`（registry 23→24），審核規則第4版（2026-09-16） |
